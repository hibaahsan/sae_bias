...
#Adopted from https://github.com/saprmarks/feature-circuits/attribution.py
...
def compute_interpolation(clean_state, patch_state, alpha):
    ...
    for submodule in submodules:
        ...
        with model.trace() as tracer:
            ...
            for step in range(steps):
                alpha = step / steps
                f = (1 - alpha) * clean_state + alpha * patch_state
                # f = checkpoint.checkpoint(compute_interpolation, clean_state, patch_state, alpha)
                f.act.requires_grad_().retain_grad()
                f.res.requires_grad_().retain_grad()
                fs.append(f)
                with tracer.invoke(clean):
                    # print(len(model.tokenizer(clean)['input_ids'][0]))
